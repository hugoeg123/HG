...
const AIAssistant = () => {
  ...
  const loadModels = async () => {
    try {
      const payload = await aiService.getModels();
      ...
      const normalized = rawModels
        ...
        .map((m) => {
          ...
          const isOllamaTag = Boolean(m.model || m.digest || m.details);
          const provider = m.provider || (isOllamaTag ? 'ollama' : undefined);
          const baseName = m.name || m.id || m.model;
          const name = provider === 'ollama' && baseName && !String(baseName).includes('(Local)')
            ? `${baseName} (Local)`
            : baseName;
      ...
      const preferred =
        sorted.find((m) => m.provider === 'ollama') ||
        sorted.find((m) => m.id === 'gpt-4') ||
        sorted[0];

      if (preferred?.id) setModel(preferred.id);
    } catch (error) {
  ...
  useEffect(() => {
    scrollToBottom();
  }, [messages, messages.length, isStreaming]); // Scroll on new messages or streaming updates
  ...
  const insertContextPill = (context) => {
    ...
    pill.className = `inline-flex items-center gap-1.5 px-2 py-0.5 mx-1 rounded-md text-xs font-medium select-none align-middle transition-colors cursor-default ${isDarkMode
  ...
  const sendMessage = async (e) => {
    ...
    const initialAiMessage = {
      ...
      sender: 'ai',
      timestamp: new Date(),
      model: model || availableModels[0]?.id || 'gpt-4',
      isStreaming: true
    };
    ...
    try {
      ...
      // Setup timeout for first token
      clientTimeout = setTimeout(() => {
        if (!hasReceivedContent) {
          if (abortControllerRef.current) abortControllerRef.current.abort();
          console.warn('Timeout waiting for first token');
        }
      }, 45000); // 45s timeout for first token (Ollama can be slow to load)
      ...
      setMessages((prev) => {
        ...
        const resolvedMessage =
          normalized.includes('failed to connect to ollama')
            ? 'Ollama indisponível. Inicie o serviço e tente novamente.'
          ...
          <div className="flex items-center justify-between px-2 pb-2">
            ...
            {isStreaming || isLoading ? (
              ...
              <button
                ...
                className={`
                            p-2 rounded-lg transition-all duration-200
                            ${!input && messages.length === 0 // Allow sending if empty but context added? Handled in sendMessage check
