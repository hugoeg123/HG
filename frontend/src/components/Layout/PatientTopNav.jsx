 ...
 */
const PatientTopNav = ({ onToggleLeftSidebar, onToggleRightSidebar, rightSidebarCollapsed }) => {
  ...
  return (
    <nav
      className={`px-4 py-2 flex items-center justify-between transition-all duration-200 ${isDarkMode
    ...
    >
      {/* Toggle Left Sidebar + Logo / Nome */}
      <div className="flex items-center gap-2">
        <button
          onClick={onToggleLeftSidebar}
          className={`p-2 rounded-lg border transition-all duration-200 focus:outline-none ${isDarkMode
            ? 'bg-theme-background text-gray-300 hover:bg-theme-surface hover:text-white border-gray-700'
            : 'bg-[#DDDDDD] text-gray-700 hover:bg-white hover:text-gray-900 border-gray-300'
            }`}
          aria-label="Alternar barra lateral esquerda"
          title="Ocultar/Exibir barra lateral esquerda"
        >
          <Menu size={16} />
        </button>
        <Link to="/" className="flex items-center" title="Ir para o Dashboard">
          <div
            className={`h-7 w-7 rounded-lg flex items-center justify-center font-bold text-lg ${isDarkMode ? 'bg-teal-500 text-theme-background' : 'bg-blue-600 text-theme-background'
              }`}
          >
            HG
          </div>
      ...
      <div className="flex items-center space-x-3">
        ...
        <button
          ...
          title={rightSidebarCollapsed ? 'Exibir barra lateral direita' : 'Ocultar barra lateral direita'}
        >
        ...
        <div className="relative">
          <button
            ...
            className={`flex items-center space-x-2 p-1.5 rounded-lg transition-all duration-200 border border-transparent focus:outline-none ${isDarkMode
          ...
          >
            <div
              className={`w-7 h-7 rounded-full border flex items-center justify-center font-medium ${isDarkMode ? 'bg-theme-background border-gray-600/20 text-gray-300' : 'bg-[#DDDDDD] border-gray-400/30 text-gray-700'
                }`}
            >
          ...
          {dropdownOpen && (
            <div
              className={`absolute right-0 mt-2 w-48 rounded-lg shadow-lg py-1 z-50 border ${isDarkMode ? 'bg-theme-card border-gray-700' : 'bg-white border-gray-200'
                }`}
            >
              {isAuthenticated ? (
                <>
                  <Link
                    to={profileHref}
                    className={`block px-4 py-2 text-sm transition-all duration-200 flex items-center space-x-2 ${isDarkMode ? 'text-gray-300 hover:bg-theme-surface hover:text-white' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900'
                      }`}
                  ...
                  <button
                    ...
                    className={`block w-full text-left px-4 py-2 text-sm border border-transparent rounded transition-all duration-200 flex items-center space-x-2 hover:bg-red-600/20 hover:text-red-300 ${isDarkMode ? 'bg-theme-card text-gray-300' : 'bg-white text-gray-700'
                      }`}
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <Link
                  to="/login"
                  className={`block px-4 py-2 text-sm transition-all duration-200 ${isDarkMode ? 'text-gray-300 hover:bg-theme-surface hover:text-white' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900'
                    }`}
                  onClick={() => setDropdownOpen(false)}
                >
                  {t('layout.login')}
