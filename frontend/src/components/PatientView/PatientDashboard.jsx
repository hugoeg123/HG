...
const TimelineItem = ({ item }) => {
    ...
    return (
      <div className="relative pl-8 py-2">
        <div className="bg-red-900/20 border border-red-500/50 rounded-lg p-4">
          <p className="text-red-400">{t('patientDashboard.timelineItemError')}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="relative pl-8 py-2 group">
      ...
      <div
        className={`theme-card p-3 sm:p-4 rounded-lg border theme-border transition-all hover:border-teal-500/50 hover:bg-theme-surface ${isClickable ? 'cursor-pointer' : ''
          }`}
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 sm:gap-4">
          <div className="flex-1">
            ...
            <p className="text-xs theme-text-muted mt-1">
              {t('patientDashboard.registeredBy')}: {safeText(item.doctorName) || t('patientDashboard.unknownDoctor')}
                  ...
                  .map((tag, index) => {
                    ...
                    return (
                      <span
                        ...
                        className={`inline-flex items-center px-2.5 py-0.5 text-[11px] sm:text-xs leading-4 font-semibold uppercase tracking-wide rounded whitespace-nowrap ${['HDA', 'QP'].includes(tagCode)
  ...
  const handleSaveName = async () => {
    if (editedName.trim() && editedName !== safeCurrentPatient?.name) {
      try {
        ...
        toast.success(t('patientDashboard.nameUpdated'));
      } catch (error) {
        console.error('Erro ao atualizar nome:', error);
        toast.error(t('patientDashboard.errorUpdatingName'));
      }
    }
  ...
  const handleSaveBirthDate = async () => {
    if (editedBirthDate && editedBirthDate !== safeCurrentPatient?.birthDate) {
      try {
        ...
        toast.success(t('patientDashboard.birthDateUpdated'));
        ...
        toast.error(t('patientDashboard.errorUpdatingBirthDate'));
      }
    }
  ...
  const loadDashboard = useCallback(async (showToast = false) => {
    ...
    try {
      ...
      if (showToast && isMountedRef.current) {
        toast.success(t('patientDashboard.dashboardUpdated'), {
          description: t('patientDashboard.patientDataLoaded')
        });
      }
    } catch (err) {
      ...
      if (isMountedRef.current) {
        toast.error(t('patientDashboard.errorLoadingDashboard'), {
          description: 'Não foi possível carregar os dados do paciente'
        });
      }
    }
  }, [safePatientId, createSignal, fetchPatientDashboard, toast, t]);
    ...
    safeCurrentPatient.records.forEach(record => {
      ...
      try {
        ...
        const recordData = {
          id: record.id,
          recordId: record.id,
          title: `${t('patientDashboard.medicalRecord')} - ${new Date(record.createdAt || record.date).toLocaleDateString('pt-BR')}`,
          content: record.content,
          allTags: allTags,
          tags: allTags,
          createdAt: record.createdAt || record.date || null,
          data: record.createdAt ? new Date(record.createdAt).toLocaleDateString('pt-BR') : (record.date ? new Date(record.date).toLocaleDateString('pt-BR') : t('patientDashboard.dateNotInformed')),
          ...
          contexto: record.context || t('patientDashboard.medicalRecord'),
          ...
          tipo: t('patientDashboard.medicalRecord'),
        ...
        categorizedData.timeline.push({
          id: record.id,
          recordId: record.id,
          title: t('patientDashboard.medicalRecord'),
          content: record.content,
          allTags: [],
          data: record.createdAt ? new Date(record.createdAt).toLocaleDateString('pt-BR') : t('patientDashboard.dateNotInformed'),
          hora: record.createdAt ? new Date(record.createdAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }) : '--:--',
          contexto: record.context || t('patientDashboard.medicalRecord'),
          medico: record.doctorName
            ? `${record.doctorName}${record.doctorCRM ? ` (CRM: ${record.doctorCRM})` : ''}`
            : t('patientDashboard.unknownDoctor'),
          tipo: t('patientDashboard.medicalRecord'),
          descricao: record.content.substring(0, 150) + (record.content.length > 150 ? '...' : ''),
          recordLink: `/records/${record.id}`
        });
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-64 bg-theme-background text-white">
        ...
        <span className="ml-2 text-gray-400">{t('patientDashboard.loadingDashboard')}</span>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-64 space-y-4 bg-theme-background text-white">
        ...
        <div className="text-center">
          <h3 className="text-lg font-semibold text-white">{t('patientDashboard.errorLoadingDashboard')}</h3>
          <p className="text-gray-400 mt-1">{error}</p>
          <button
            onClick={handleRetry}
            className="btn btn-primary mt-4"
          >
            {t('patientDashboard.retry')}
          </button>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-theme-background text-gray-300 font-sans p-2 sm:p-4 lg:p-8">
        <div className="max-w-7xl mx-auto">
          <div className="bg-theme-surface p-6 rounded-lg border border-gray-800">
            <p className="text-gray-500">{t('patientDashboard.noPatientSelected')}</p>
          </div>
        </div>
      </div>
    );
        ...
        <header className="flex flex-col sm:flex-row items-start sm:items-center pb-3 sm:pb-4 mb-4 sm:mb-6 lg:mb-8 border-b border-gray-800 gap-3 sm:gap-4">
          ...
          <div className="flex-1 w-full sm:w-auto">
              ...
              {isEditingName ? (
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    title={t('patientDashboard.cancelEdit')}
                  >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                  ...
                  <button
                    ...
                    title={t('patientDashboard.editName')}
                  >
              ...
              {isEditingBirthDate ? (
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    title={t('patientDashboard.saveBirthDate')}
                  >
                    <Save size={16} />
                  </button>
                  <button
                ...
                <div className="flex items-center gap-2">
                  <h2 className="text-xl font-bold text-white">
                    {typeof patientAge === 'number' ? t('dashboard.age', { count: patientAge }) : t('dashboard.age', { count: 0 })}
                  </h2>
                  <button
                    onClick={handleEditBirthDate}
                    className="button-exception p-1 text-foreground hover:text-foreground transition-colors"
                    title={t('patientDashboard.editBirthDate')}
                  >
                    <Edit2 size={16} />
                  </button>
                </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 text-xs sm:text-sm text-gray-500">
              <span>{t('patientDashboard.id')}: {typeof safeCurrentPatient?.id === 'string' || typeof safeCurrentPatient?.id === 'number' ? safeCurrentPatient.id : 'N/A'}</span>
              <span className="hidden sm:inline">|</span>
              <span>{t('patientDashboard.recordNumber')}: {typeof safeCurrentPatient?.recordNumber === 'string' || typeof safeCurrentPatient?.recordNumber === 'number' ? safeCurrentPatient.recordNumber : 'N/A'}</span>
              {safeCurrentPatient?.birthDate && (
                <>
                  <span className="hidden sm:inline">|</span>
                  <span>{t('patientDashboard.birthDate')}: {new Date(safeCurrentPatient.birthDate).toLocaleDateString('pt-BR')}</span>
                </>
              )}
            </div>
          </div>
        ...
        <main className="flex flex-col xl:flex-row gap-4 sm:gap-6 xl:gap-8">
            ...
            <div className="patient-dashboard-panel p-3 sm:p-4 rounded-lg border border-gray-700/30 flex flex-col gap-3 sm:gap-4 bg-theme-background overflow-hidden">
              ...
              <div className="flex items-center justify-center overflow-x-auto">
                <div className="flex flex-row items-stretch gap-1 sm:gap-2 p-0 bg-theme-card rounded-lg min-w-max">
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">{t('patientDashboard.timeline')}</span>
                    <span className="sm:hidden">{t('patientDashboard.history')}</span>
                  </button>
                  <button
                    onClick={() => setActiveTab('investigacao')}
                    className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-3 md:px-4 py-2 sm:py-3 text-xs sm:text-sm font-semibold rounded-lg transition-all duration-200 border min-w-[80px] sm:min-w-[100px] md:min-w-[120px] h-[32px] sm:h-[36px] md:h-[40px] justify-center whitespace-nowrap focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-theme-background ${activeTab === 'investigacao'
                        ? 'border-teal-600 border-2 bg-teal-600/20 text-teal-300'
                        : 'bg-theme-card text-gray-300 hover:bg-theme-surface hover:text-white border-transparent'
                      }`}
                  >
                    <Beaker size={14} className="sm:w-4 sm:h-4 flex-shrink-0" />
                    <span className="hidden sm:inline">{t('patientDashboard.exams')}</span>
                    <span className="sm:hidden">{t('patientDashboard.investigation')}</span>
                  </button>
                  <button
                    onClick={() => setActiveTab('planos')}
                    className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-3 md:px-4 py-2 sm:py-3 text-xs sm:text-sm font-semibold rounded-lg transition-all duration-200 border min-w-[80px] sm:min-w-[100px] md:min-w-[120px] h-[32px] sm:h-[36px] md:h-[40px] justify-center whitespace-nowrap focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-theme-background ${activeTab === 'planos'
                        ? 'border-teal-600 border-2 bg-teal-600/20 text-teal-300'
                        : 'bg-theme-card text-gray-300 hover:bg-theme-surface hover:text-white border-transparent'
                      }`}
                  >
                    <NotebookPen size={14} className="sm:w-4 sm:h-4 flex-shrink-0" />
                    <span className="hidden sm:inline">{t('patientDashboard.plans')}</span>
                    <span className="sm:hidden">{t('patientDashboard.pln')}</span>
                  </button>
                </div>
              </div>
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 px-2 sm:px-0">
                <div className="relative flex-1 min-w-0">
                  ...
                  <input
                    type="text"
                    placeholder={t('patientDashboard.searchTimeline')}
                    value={searchTerm}
                ...
                <button className="px-3 sm:px-4 py-2 bg-theme-card text-gray-300 rounded-lg hover:bg-theme-surface hover:text-white transition-colors flex items-center gap-2 text-sm sm:text-base justify-center border border-transparent hover:border-teal-500/30 whitespace-nowrap flex-shrink-0">
                  ...
                  <span className="hidden sm:inline">{t('patientDashboard.filters')}</span>
                  <span className="sm:hidden">{t('patientDashboard.filter')}</span>
                </button>
              </div>
            </div>
              ...
              {activeTab === 'historico' && (
                filteredData.length === 0 ? (
                  <div className="text-center py-8 sm:py-16 text-gray-500 px-4">
                    <p className="text-sm sm:text-base">{t('patientDashboard.noItemsFound')}</p>
                    <p className="text-xs sm:text-sm mt-1">{t('patientDashboard.adjustSearch')}</p>
                  </div>
                ) : (
              ...
              {activeTab === 'planos' && (
                filteredData.length === 0 ? (
                  <div className="text-center py-16 text-gray-500">
                    <p>{t('patientDashboard.noItemsFound')}</p>
                    <p className="text-sm">{t('patientDashboard.adjustSearch')}</p>
                  </div>
                ) : (
          ...
          <aside className="lg:w-80 xl:w-96 flex-shrink-0 space-y-4 sm:space-y-6 xl:space-y-8 order-2 lg:order-2">
            ...
            >
              ...
              <span className="hidden md:inline">{t('patientDashboard.startNewRecord')}</span>
              <span className="md:hidden">{t('patientDashboard.newRecord')}</span>
            </button>
            ...
            <div className="patient-dashboard-card bg-theme-card p-3 sm:p-4 rounded-lg border border-gray-800 transition-all hover:border-teal-500/50 hover:bg-theme-card/80 cursor-pointer">
              <h3 className="font-semibold text-base sm:text-lg theme-text-primary mb-3 sm:mb-4">{t('patientDashboard.recentVitals')}</h3>
              {isLoading ? (
              ...
              {isLoading ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="pt-4 border-t border-gray-800">
                    <p className="text-sm font-semibold text-gray-400 flex items-center gap-2 mb-2"><AlertTriangle size={16} /> {t('patientDashboard.allergies')}</p>
                    {realData.alergias.length > 0 ? (
                      ...
                      <p className="text-sm text-gray-500">{t('patientDashboard.noAllergies')}</p>
                    )}
                  </div>
                  <div className="pt-4 border-t border-gray-800">
                    <p className="text-sm font-semibold text-gray-400 flex items-center gap-2 mb-2"><Pill size={16} /> {t('patientDashboard.medications')}</p>
                    {realData.medicamentosEmUso.length > 0 ? (
                      ...
                      <p className="text-sm text-gray-500">{t('patientDashboard.noMedications')}</p>
                    )}
                  </div>
                </div>
              )}
...
const HistoryList = ({ data, patientId }) => {
  ...
  if (!data || data.length === 0) {
    return (
      <div className="patient-dashboard-card bg-theme-card p-6 rounded-lg border border-gray-800">
        <div className="text-center text-gray-500">
          <FileText className="w-12 h-12 mx-auto mb-4 text-gray-300" />
          <p>{t('patientDashboard.noHistoryFound')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="patient-dashboard-card bg-theme-card p-6 rounded-lg border border-gray-800">
        <div className="text-center text-gray-500">
          ...
          <p>{t('patientDashboard.noInvestigationFound')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="patient-dashboard-card bg-theme-card p-6 rounded-lg border border-gray-800">
        <div className="text-center text-gray-500">
          ...
          <p>{t('patientDashboard.noPlanFound')}</p>
        </div>
      </div>
    );
  }
...
const RecordCard = ({ record, type, patientId }) => {
  ...
  const formatDate = (dateString) => {
    if (!dateString) return t('patientDashboard.dateNotInformed');
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 sm:gap-4">
        <div className="flex-1">
          <p className="text-xs sm:text-sm text-gray-400">
            {formatDate(createdAtValue)} • {t('patientDashboard.medicalRecord')}
          </p>
          <div className="flex items-start gap-2 mt-1">
            <h3 className="font-semibold text-sm sm:text-base text-white flex-1">
              {record.title || `${t('patientDashboard.medicalRecord')} - ${formatDate(createdAtValue)}`}
            </h3>
            <ArrowRight size={14} className="text-teal-400 mt-1 flex-shrink-0" aria-hidden="true" />
          </div>
          ...
          {(record.doctorName || record.doctorCRM) && (
            <p className="text-xs text-gray-500 mt-1">
              {(() => {
                ...
                if (name) return `${t('patientDashboard.registeredBy')}: ${crm ? `${name} (CRM ${crm})` : name}`;
                return `${t('patientDashboard.registeredBy')}: ${crm ? `CRM ${crm}` : t('patientDashboard.unknownDoctor')}`;
              })()}
            </p>
          )}
        ...
        <div className="text-teal-400 flex-shrink-0 mt-1 self-start sm:self-auto" title={t('patientDashboard.medicalRecord')}>
